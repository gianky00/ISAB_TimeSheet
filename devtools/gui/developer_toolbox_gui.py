...
class DeveloperToolboxGUI(QMainWindow):
    ...
    def _full_audit(self) -> None:
        ...
        self._run_command([str(VENV_PYTHON), "devtools/gui/pre_flight_check.py"], "Full Audit")

    def _fast_audit(self) -> None:
        """Opzione 2: Fast Audit"""
        self._run_command([str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--fast"], "Fast Audit")

    def _smart_check(self) -> None:
        """Opzione 3: Smart Check"""
        self._run_command(
            [str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--fast", "--inc"],
            "Smart Check (Incremental)",
        )

    def _only_tests(self) -> None:
        """Opzione 4: Only Tests"""
        self._run_command([str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--test-only"], "Only Tests")

    def _auto_fix(self) -> None:
        """Opzione 5: Auto-Fix"""
        self._run_command(
            [str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--fix", "--fast"],
            "Auto-Fix (Ruff)",
        )

    def _dashboard(self) -> None:
        """Opzione 6: Dashboard"""
        dashboard_path = PROJECT_ROOT / ".cache" / "reports" / "preflight" / "dashboard.html"
    ...
    def _security_scan(self) -> None:
        ...
        self._run_command(
            [str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--target", "security"],
            "Security Scan",
        )
    ...
    def _coverage(self) -> None:
        """Opzione 8: Coverage"""
        self._run_command(
            [str(VENV_PYTHON), "-m", "tests.run_robust_test", "--cov=src", "--cov-report=html:.cache/htmlcov"],
            "Code Coverage",
        )
    ...
    def _architecture(self) -> None:
        ...
        self._run_command(
            [str(VENV_PYTHON), "devtools/cli/generate_architecture.py"],
    ...
    def _emergency_setup(self) -> None:
        ...
        build_script = str(PROJECT_ROOT / "devtools" / "gui" / "Crea Setup" / "build_dist.py")
        self._run_command(
    ...
    def _secrets_mgmt(self) -> None:
        ...
        self._run_command([str(VENV_PYTHON), "devtools/gui/manage_secrets_gui.py"], "Secrets Management GUI")
    ...
    def _db_maintain(self) -> None:
        ...
        self._run_command([str(VENV_PYTHON), "devtools/maintenance/db_maintenance.py"], "Database Maintenance")

    def _raw_metrics(self) -> None:
        """Opzione 20: Raw Metrics"""
        self._run_command(
            [str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--target", "metrics"],
            "Raw Metrics",
        )
    ...
    def _clean_code(self) -> None:
        ...
        self._run_command(
            [str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--target", "clean"],
            "Clean Code",
        )
    ...
    def _depty_check(self) -> None:
        ...
        self._run_command(
            [str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--target", "deps"],
            "Dependency Check",
        )
    ...
    def _project_stats(self) -> None:
        ...
        self._run_command(
            [str(VENV_PYTHON), "devtools/gui/pre_flight_check.py", "--target", "stats"],
            "Project Stats",
        )
    ...
    def _clean_cache(self) -> None:
        ...
        self._run_command([str(VENV_PYTHON), "devtools/maintenance/clean_venv.py"], "Clean Cache")

    def _inspector(self) -> None:
        """Opzione 26: Inspector"""
        self._run_command([str(VENV_PYTHON), "devtools/maintenance/universal_inspector.py"], "Universal Inspector")

    def _dep_audit(self) -> None:
        """Opzione 27: Dependency Audit"""
        self._run_command([str(VENV_PYTHON), "devtools/maintenance/analyze_dependencies.py"], "Dependency Audit")
