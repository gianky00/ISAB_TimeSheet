...
def test_get_detailed_license_status_missing_dir(mocker, tmp_path):
    ...
    mocker.patch("os.path.exists", side_effect=lambda p: p != mock_paths["dir"] and p != mock_paths["config"] and p != mock_paths["manifest"])
...
def test_get_detailed_license_status_missing_files(mocker, mock_license_dir):
    ...
    mocker.patch("os.path.exists", side_effect=lambda p: p != os.path.join(mock_license_dir, "non_existent.dat") and p != os.path.join(mock_license_dir, "non_existent.json"))
    status, msg = get_detailed_license_status()
...
def test_get_detailed_license_status_valid(mocker, mock_license_dir, mock_secrets_manager, setup_valid_license_files):
    mocker.patch("src.core.license_validator._get_license_paths", return_value={
        "dir": mock_license_dir,
        "config": os.path.join(mock_license_dir, "config.dat"),
        "manifest": os.path.join(mock_license_dir, "manifest.json")
    ...
    mocker.patch("src.core.license_validator.get_hardware_id", return_value="FAKE_HW_ID")
    mocker.patch("src.core.time_manager.get_trusted_time", return_value=(datetime.now(timezone.utc) - timedelta(days=1), True))
...
def test_verify_license(mocker):
    mocker.patch("src.core.license_validator.get_detailed_license_status", return_value=(LicenseStatus.VALID, "OK"))
