...
class TestContabilitaStatsDeep:
    def test_get_year_stats_calculations(self, mocker):
        ...
        mock_data = [
            ("01/01", "Gen", "P1", "1.000,00", "Attivita A", "T1", "O1", "COMPLETATO", "T", "8,0", "1", "N", "P", "F"),
            ("02/01", "Gen", "P2", "500,50", "Attivita B", "T1", "O2", "IN CORSO", "T", "4,5", "1", "N", "P", "F"),
            ("03/01", "Gen", "TOTALE", "1.500,50", "IGNORE", "T1", "O3", "DONE", "T", "12,5", "1", "N", "P", "F"), # Deve essere ignorata
        ]
        
        mocker.patch("src.core.contabilita_queries.ContabilitaQueries.get_data_by_year", return_value=mock_data)
        mocker.patch("src.core.contabilita_queries.ContabilitaQueries.get_giornaliere_by_year", return_value=[])
    ...
    def test_get_year_stats_top_commesse(self, mocker):
        ...
        mock_data = [
            ("D", "M", f"P{i}", f"{i*100}", f"Att {i}", "T", "O", "S", "T", "0", "R", "N", "P", "F")
    ...
    def test_get_year_stats_dirette_indirette(self, mocker):
        ...
        mock_giornaliere = [
            ("D1", "U1", "T1", "Desc", "P123", "", "PDL", "08", "17", "9,0", "F1"), # DIRETTA (ha n_prev)
            ("D2", "U1", "T1", "Desc", "", "ODC456", "PDL", "08", "17", "4,5", "F1"), # DIRETTA (ha odc)
            ("D3", "U1", "T1", "Indiretta", "", "", "", "08", "17", "2,0", "F1"),    # INDIRETTA (vuote)
            ("D4", "U1", "T1", "Desc", "NaN", "nan", "", "08", "17", "1,0", "F1"), # INDIRETTA (NaN stringhe)
        ]
        
        mocker.patch("src.core.contabilita_queries.ContabilitaQueries.get_data_by_year", return_value=[])
        mocker.patch("src.core.contabilita_queries.ContabilitaQueries.get_giornaliere_by_year", return_value=mock_giornaliere)
