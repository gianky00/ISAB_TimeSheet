...
def test_import_contabilita_dati_success(mock_xls_file):
    mock_cb = MagicMock()
    success, msg, rows, years = ExcelImporter.import_contabilita_dati(str(mock_xls_file), progress_callback=mock_cb)
...
def test_import_contabilita_dati_critical_error():
    with patch("src.core.excel_importer.pd.ExcelFile", side_effect=Exception("Critical")):
...
def test_import_giornaliere_success(tmp_path):
    ...
    with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
        ...
        pd.DataFrame({"DATA": ["Totale"]}).to_excel(writer, sheet_name="RIASSUNTO", startrow=2, index=False, header=False)
    ...
    with patch("src.core.excel_importer.ProcessPoolExecutor") as mock_pool:
        ...
        success, msg, rows, years = ExcelImporter.import_giornaliere(str(root), lookup_map, progress_callback=mock_cb)
        assert success is True
        assert 2025 in years
        assert len(rows) > 0
...
def test_process_single_giornaliera_extraction_logic(tmp_path):
    ...
    df = pd.DataFrame({
        ...
        "PERSONALE": ["A", "B", "C"],
        "DESCRIZIONE ATTIVITA'": ["Commessa 22/123", "Standard 540012345", "Canone mensile"],
...
def test_import_scarico_ore_success(tmp_path):
    ...
    # Header row (6)
    # col 2 to 12
    headers = ["Data", "Pers1", "Pers2", "ODC", "Pos", "Dalle", "Alle", "Tot", "Desc", "Finito", "Comm"]
    for i, h in enumerate(headers, start=2):
        ws.cell(row=5, column=i, value=h)

    # Data row (6)
    data = ["01/01/2025", "U1", "U2", "54001", "10", "08:00", "17:00", "8.0", "Desc", "Sì", "C1"]
...
def test_process_scarico_ore_row_validation():
    ...
    row_0_odc = [MockCell("date"), MockCell("P1"), MockCell(""), MockCell(0), MockCell("10"), MockCell(""), MockCell(""), MockCell("8"), MockCell(""), MockCell(""), MockCell("")]
    ...
    row_no_pers = [MockCell("date"), MockCell(""), MockCell(""), MockCell("5400"), MockCell("10"), MockCell(""), MockCell(""), MockCell("8"), MockCell(""), MockCell(""), MockCell("")]
    res = ExcelImporter._process_scarico_ore_row(row_no_pers, col_keys)
    assert res is None

    # 3. Row with only partial required (missing pos)
    row_no_pos = [MockCell("date"), MockCell("P1"), MockCell(""), MockCell("5400"), MockCell(""), MockCell(""), MockCell(""), MockCell("8"), MockCell(""), MockCell(""), MockCell("")]
