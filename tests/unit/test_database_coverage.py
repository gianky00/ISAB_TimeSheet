...
class TestDatabaseManager:
    ...
    def manager(self, tmp_path, mocker):
        ...
        mocker.patch("src.core.database.DatabaseManager.DB_CONTABILITA", tmp_path / "contabilita.db")
        mocker.patch("src.core.database.DatabaseManager.DB_TIMBRATURE", tmp_path / "timbrature.db")
    ...
    def test_execute_query_select(self, manager):
        manager.init_db()
        # Insert test data
        manager.execute_query(manager.DB_CONTABILITA, 
    ...
    def test_execute_query_retry_on_busy(self, manager):
        ...
        with patch("sqlite3.connect", side_effect=[
