...
class TestLogHumanizerCoverage:
    ...
    def test_humanize_search(self):
        ...
        human, _, cat = SmartLogTranslator.humanize(msg)
        assert human == "[CERCA] Ricerca in corso..."
        assert cat == "search"
    ...
    def test_humanize_download(self):
        ...
        msg = "scarico i file"
        human, _, cat = SmartLogTranslator.humanize(msg)
        assert human == "[DOWNLOAD] Scarico file in corso..."
        assert cat == "download"
    ...
    def test_humanize_error(self):
        ...
        msg = "errore critico"
        human, _, cat = SmartLogTranslator.humanize(msg)
        assert human == "[ERRORE] Errore critico rilevato!"
        assert cat == "error"
    ...
    def test_detect_category_error_variants(self):
        ...
        assert SmartLogTranslator._detect_category("[ERRORE] Portale offline") == "error"
    ...
    def test_detect_category_success_variants(self):
        ...
        assert SmartLogTranslator._detect_category("[OK] Dati inviati") == "success"
        assert SmartLogTranslator._detect_category("[INFO] Risultato ottimo") == "success"

    def test_humanize_preserve_custom_icons(self):
        """Verifica che icone non standard vengano preservate."""
        msg = "[INFO] Nota di sistema"
