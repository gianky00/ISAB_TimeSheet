...
class TestTimeManager:
    ...
    def test_get_network_time_success(self, mock_head):  # noqa: ANN001
        ...
        assert result.month == 2  # noqa: PLR2004
        assert result.day == 1
        assert result.tzinfo is not None

    @patch("src.core.time_manager.requests.head")
    def test_get_network_time_failure(self, mock_head):  # noqa: ANN001
        mock_head.side_effect = Exception("Connection error")
        result = get_network_time()
        assert result is None

    @patch("src.core.time_manager.get_network_time")
    def test_get_trusted_time_trusted(self, mock_net):  # noqa: ANN001
