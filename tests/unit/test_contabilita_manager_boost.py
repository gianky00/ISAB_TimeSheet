...
class TestContabilitaManagerBoost:
    ...
    def test_cleanup_future_years(self, db_setup):
        ...
        count_cont = manager.execute_query(db_path, "SELECT COUNT(*) FROM contabilita WHERE year >= 2026")[0][
            0
        ]
        count_giorn = manager.execute_query(db_path, "SELECT COUNT(*) FROM giornaliere WHERE year >= 2026")[
            0
        ][0]
        assert count_cont == 0
