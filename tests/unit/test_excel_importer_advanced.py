...
class TestExcelImporterAdvanced:
    ...
    def test_import_contabilita_dati_mapping_resilience(self, mock_xls, mocker):
        ...
        mocker.patch("src.core.excel_importer.ExcelImporter._get_excel_file", return_value=mock_xls)
        ...
        mocker.patch("src.core.excel_importer.pd.read_excel", side_effect=[
            ...
            pd.DataFrame(columns=["DATA PREV", "MESE", "N PREV", "TOTALE PREV", "ATTIVITA", "ODC"]) # Actual 2 (Empty)
        ])
        mocker.patch("src.core.excel_importer.ExcelImporter._decrypt_if_encrypted", return_value=("fake_path", False))
    ...
    def test_process_single_giornaliera_cleaning(self):
        ...
        with patch("src.core.excel_importer.pd.read_excel", return_value=df_giorn):
            year, rows, err = ExcelImporter._process_single_giornaliera((2026, Path("test.xlsx"), {}))
            assert err is None
            assert len(rows) == 2
    ...
    def test_giornaliera_odc_lookup(self):
        ...
        with patch("src.core.excel_importer.pd.read_excel", return_value=df):
            year, rows, err = ExcelImporter._process_single_giornaliera((2026, Path("f.xlsx"), lookup))
            assert len(rows) > 0
