...
def test_load_base_config_exception(tmp_path, mocker):
    ...
    mocker.patch("src.application.services.config_manager.CONFIG_FILE", f)
...
def test_set_default_account_fallback(mocker):
    mocker.patch("src.application.services.config_manager.set_default_account_logic", return_value=False)
...
def test_import_config_from_file_exceptions(tmp_path, mocker):
    ...
    mocker.patch("src.application.services.config_manager.save_config", return_value=False)
    success, msg = import_config_from_file(f, async_save=False)
    assert success is False
