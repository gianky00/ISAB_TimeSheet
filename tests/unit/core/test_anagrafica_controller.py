...
class TestAnagraficaController:
    ...
    def test_get_employees_search_terms_logic(self, mock_db):  # noqa: ANN001
        ...
        assert query.count("AND (cognome LIKE ?") == 2  # noqa: PLR2004
        assert len(params) == 8  # noqa: PLR2004
        assert "%rossi%" in params
        assert "%mario%" in params

    @patch("src.core.dipendenti.anagrafica_controller.db_manager")
    def test_toggle_monitoring_error_handling(self, mock_db):  # noqa: ANN001
    ...
    def test_process_rows_full_cycle(self, mock_compute, mock_db):  # noqa: ANN001
