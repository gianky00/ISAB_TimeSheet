...
from src.application.services.license_hwid import (
...
class TestLicenseHWID:
    @patch("src.application.services.license_hwid.platform.system", return_value="Windows")
    @patch("src.application.services.license_hwid._get_windows_hardware_id")
    ...
    @patch("src.application.services.license_hwid.platform.system", return_value="Linux")
    @patch("src.application.services.license_hwid._get_linux_hardware_id")
    ...
    @patch("src.application.services.license_hwid.platform.system", return_value="Darwin")
    @patch("src.application.services.license_hwid.uuid.getnode", return_value=123456789)
    def test_get_hardware_id_fallback(self, mock_uuid, mock_system):
        assert get_hardware_id() == "123456789"

    @patch("src.application.services.license_hwid.platform.system", return_value="Darwin")
    @patch("src.application.services.license_hwid.uuid.getnode", side_effect=Exception("Error"))
    def test_get_hardware_id_fallback_error(self, mock_uuid, mock_system):
        assert get_hardware_id() == "ERROR_GETTING_ID"

    @patch("src.application.services.license_hwid.subprocess.check_output")
    ...
    @patch("src.application.services.license_hwid.subprocess.check_output", side_effect=Exception("Err"))
