...
def test_parse_currency_basics():
    ...
    assert parse_currency(123) == 123.0  # noqa: PLR2004
...
def test_parse_currency_symbols():
    ...
    assert parse_currency("1.234,56 €") == 1234.56  # noqa: PLR2004
    assert parse_currency("1234,56 Euro") == 1234.56  # noqa: PLR2004
    assert parse_currency("EURO 1234,56") == 1234.56  # noqa: PLR2004


def test_parse_currency_negative():
    assert parse_currency("-100,50") == -100.5  # noqa: PLR2004
    assert parse_currency("100,50-") == -100.5  # noqa: PLR2004
    assert parse_currency("100,50 - ") == -100.5  # noqa: PLR2004
    assert parse_currency(" - 100,50") == -100.5  # noqa: PLR2004
...
def test_parse_currency_formats():
    ...
    assert parse_currency("1,234") == 1.234  # Current logic converts , to .  # noqa: PLR2004

    # Only dot
    assert parse_currency("1234.56") == 1234.56  # noqa: PLR2004
    assert parse_currency("1.234.567") == 1234567.0  # noqa: PLR2004
    assert parse_currency("1.234") == 1234.0  # Punto seguito da 3 cifre = migliaia in IT  # noqa: PLR2004
    assert parse_currency("10.5") == 10.5  # noqa: PLR2004
