...
class TestReportGeneratorRobust(unittest.TestCase):
    def setUp(self):
        ...
        self.logger_patcher = patch('src.gui.panels.dipendenti.utils.report_generator.logger', self.mock_logger)
    ...
    def test_generate_email_report_success(self, mock_build_maps, mock_create_excel, mock_send_email, mock_db):
    ...
    @patch('src.gui.panels.dipendenti.utils.report_generator.ReportHistory')
    @patch('src.gui.panels.dipendenti.utils.report_generator.ToastManager')
    def test_send_report_email_fallback(self, mock_toast, mock_history, mock_write, mock_from_local, mock_open_url):
        """Test fallback when Outlook fails or is not available."""
        # Force Outlook failure by making Dispatch raise
        with patch('win32com.client.Dispatch', side_effect=Exception("Outlook not found"), create=True):
            with patch('src.gui.panels.dipendenti.utils.report_generator.os.name', 'nt'):
                data = {"warning_list": [], "expired_list": []}
