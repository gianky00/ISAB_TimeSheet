...
class TestTelegramUIBridge(unittest.TestCase):
    ...
    @patch("src.core.telegram_bridge.InlineKeyboardMarkup")
    @patch("asyncio.run_coroutine_threadsafe")
    def test_handle_intent_download_pdl(
        self,
        mock_run_coroutine_threadsafe,
        MockInlineKeyboardMarkup,
        MockInlineKeyboardButton,
    ...
    def test_handle_search_db_pdf_timbrature(
        self, mock_datetime, mock_exists, mock_generate_pdf, mock_config_manager
    ):
        ...
        self.mock_telegram_service.send_message_sync.assert_any_call(
            "🔍 Ricerca in corso in **timbrature** per: `test`..."
        )
        mock_generate_pdf.assert_called_once()
        self.mock_telegram_service.send_document_sync.assert_called_once_with(
            "temp/report.pdf", caption="📄 Report timbrature"
        )
    ...
    def test_handle_search_db_pdf_strumentale(
        self,
        MockContabilitaManager,
        mock_datetime,
        mock_exists,
        mock_generate_pdf,
        mock_config_manager,
        ...
        self.mock_telegram_service.send_message_sync.assert_any_call(
            "🔍 Ricerca in corso in **strumentale** per: `test`..."
        )
        mock_generate_pdf.assert_called_once()
        self.mock_telegram_service.send_document_sync.assert_called_once_with(
            "temp/report.pdf", caption="📄 Report strumentale"
        )
    ...
    def test_handle_status(self):
        ...
        self.mock_main_window._get_active_bot_panel = MagicMock(
            return_value=mock_panel
        )  # Added mock for _get_active_bot_panel
