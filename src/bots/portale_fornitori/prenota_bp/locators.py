...
class PrenotaBPLocators:
    ...
    BT_SI_SESSIONE_ATTIVA = (By.XPATH, "//span[normalize-space(text())='Si' or normalize-space(text())='Sì']/ancestor::a")
    USER_INFO_PANEL = (By.ID, "user-info-panel")

    # Menu Principale (Buono di Prelievo)
    MENU_BUONO_PRELIEVO = (By.XPATH, "//div[contains(@class, 'x-title-text')][text()='Buono di Prelievo']")

    # Sotto Menu (Gestione Buono di Prelievo)
    SUBMENU_GESTIONE_BP = (By.XPATH, "//span[contains(@class, 'x-btn-inner')][text()='Gestione Buono di Prelievo']")

    # Pannello Gestione BP (Griglia e Filtri)
    FILTER_FORNITORE = (By.NAME, "Fornitore")
    FILTER_FORNITORE_ARROW = (By.XPATH, "//div[contains(@id, 'generic_refresh_combo_box') and contains(@id, 'trigger-picker')]")
    ...
    BT_NUOVO = (By.XPATH, "//a[contains(@class, 'x-btn')][.//span[contains(normalize-space(text()), 'Nuovo')]]")


    # Form Nuova Prenotazione
    # Cerchiamo varianti comuni per i nomi dei campi nel DOM ExtJS
    CAMPO_NUMERO_BP = (By.XPATH, "//input[@name='NumeroBP' or @name='Numero' or @name='BPNumber' or contains(@name, 'Numero')]")
    CAMPO_NOTE = (By.XPATH, "//textarea[@name='NoteRitiro' or @name='Note' or @name='Notes' or contains(@name, 'Note')] | //input[@name='Note']")
