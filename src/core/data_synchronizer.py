...
class DataSynchronizer:
    ...
    @classmethod
    def sync_contabilita_dati(cls, db_path: str, imported_data: List[Tuple], imported_years: List[int]) -> Tuple[int, int]:
        """
        ...
        with db_manager.get_connection(db_path) as conn:
            ...
            for year in imported_years:
                ...
                existing_df[cols_to_str_ex] = existing_df[cols_to_str_ex].astype(str).apply(lambda x: x.str.strip())
                ...
                cursor.executemany(query, new_rows_for_year) # Use already filtered and cleaned new_rows_for_year
    ...
    @classmethod
    def sync_giornaliere(cls, db_path: str, all_new_rows: List[Tuple], years_to_clear: List[int]) -> Tuple[int, int]:
        """
