...
class SyncroJobAgent:
    ...
    async def _async_send(self, text, photo_hex=None):
        try:
            if photo_hex:
                ...
                await self.tg_app.bot.send_photo(chat_id=self.connected_chat_id, photo=photo_bytes, caption=text, parse_mode=constants.ParseMode.MARKDOWN)
            else:
                await self.tg_app.bot.send_message(chat_id=self.connected_chat_id, text=text, parse_mode=constants.ParseMode.MARKDOWN if "*" in text else None)
        except Exception as e:
    ...
    async def _cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("📊 Stato", callback_data="status"), InlineKeyboardButton("📸 Screenshot", callback_data="screenshot")],
            [InlineKeyboardButton("⚡ Power", callback_data="menu_power")]
        ]
        await update.message.reply_text("🚀 *SyncroJob Agent Attivo*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=constants.ParseMode.MARKDOWN)
    ...
    async def _handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.client_conn:
            ...
            await update.message.reply_text("⚠️ *SyncroJob è Offline*\nL'applicazione principale non è in esecuzione sul PC.", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=constants.ParseMode.MARKDOWN)
            return
    ...
    async def _process_with_ai(self, chat_id, data, is_audio=False):
        ...
        try:
            ...
            self._send_to_tg(f"❓ Comando non riconosciuto.")
    ...
    async def _handle_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "app_start_now":
            try:
                ...
                await query.edit_message_text("🔄 *Avvio in corso...*\nL'applicazione si aprirà tra pochi secondi.")
            except Exception as e:
            ...
            keyboard = [[InlineKeyboardButton("🖼️ App", callback_data="snap_app"), InlineKeyboardButton("🖥️ PC", callback_data="snap_pc")]]
