...
from src.core.config_manager import CONFIG_DIR, load_config, set_config_value
...
class BackupManager:
    ...
    def detect_cloud_paths() -> Dict[str, Path]:
        ...
        for db_path in [user_home / "Dropbox", user_home / "Dropbox (Personal)", user_home / "Dropbox (Business)"]:
            if db_path.is_dir():
    ...
    def get_backup_dir() -> Path:
        ...
        if preferred == "Local":
            target = Path.home() / "Documents" / "BotTS_Backups"
        ...
        if preferred and preferred in clouds:
            target = clouds[preferred] / "BotTS_Backups"
            target.mkdir(parents=True, exist_ok=True)
            return target
        ...
        if "OneDrive" in clouds:
            target = clouds["OneDrive"] / "BotTS_Backups"
        elif clouds:
            target = list(clouds.values())[0] / "BotTS_Backups"
        else:
            target = Path.home() / "Documents" / "BotTS_Backups"
    ...
    def create_backup() -> Tuple[bool, str]:
        ...
        try:
            ...
            zip_filename = f"BotTS_Backup_{timestamp}.zip"
    ...
    def _cleanup_old_backups(target_dir: Path, keep: int = 5):
        ...
        try:
            backups = sorted(target_dir.glob("BotTS_Backup_*.zip"), key=os.path.getmtime, reverse=True)
    ...
    def restore_backup(zip_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            AuditManager().log_action("Ripristino Backup", category="sistema", params={"file": Path(zip_path).name}, severity="high")
