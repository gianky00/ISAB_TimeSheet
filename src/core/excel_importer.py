...
class ExcelImporter:
    ...
    @classmethod
    def import_contabilita_dati(cls, file_path: str, progress_callback: Optional[Callable[[int, int], None]] = None) -> Tuple[bool, str, list, list]:
        """
            ...
            with warnings.catch_warnings():
                ...
                for sheet_name in xls.sheet_names:
                    ...
                    try:
                        ...
                        logging.warning(f"Errore durante l'elaborazione del foglio {sheet_name}: {e}")
                        continue
                ...
                return True, f"Anni importati: {sorted(list(set(imported_years)))}", all_new_rows, list(set(imported_years))

        except Exception as e:
            return False, f"Errore: {e}", [], []
    
    @classmethod
    def _process_single_giornaliera(cls, args: Tuple[int, Path, Dict]) -> Tuple[int, List[Tuple], Optional[str]]:
    ...
    @classmethod
    def import_giornaliere(cls, root_path: str, lookup_map: Dict, progress_callback: Optional[Callable[[int, int], None]] = None) -> Tuple[bool, str, List[Tuple], List[int]]:
        """
        ...
        return True, f"Importate Giornaliere: {sorted(imported_years)}", all_new_rows, imported_years

    @classmethod
    def import_attivita_programmate(cls, file_path: str, progress_callback: Optional[Callable[[int, int], None]] = None) -> Tuple[bool, str, List[Tuple]]:
        ...
        try:
            ...
            return True, f"Importate {len(rows_to_insert)} righe in Attività Programmate.", rows_to_insert

        except Exception as e:
            return False, f"Errore importazione Attività Programmate: {e}", []

    @classmethod
    def import_scarico_ore(cls, file_path: str, progress_callback: Optional[Callable[[int, int], None]] = None) -> Tuple[bool, str, List[Tuple]]:
        ...
        try:
            ...
            return True, f"Importate {len(rows_to_insert)} righe da Scarico Ore.", rows_to_insert

        except Exception as e:
            return False, f"Errore importazione Scarico Ore: {e}", []

    @classmethod
    def import_certificati_campione(cls, file_path: str, progress_callback: Optional[Callable[[int, int], None]] = None) -> Tuple[bool, str, List[Tuple]]:
            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    df = pd.read_excel(path, sheet_name=sheet_name, header=header_row_idx)
                     ...
                     return False, f"Errore lettura file Certificati (sheet: {sheet_name}): {e}", []

                if df.empty: return False, "Foglio vuoto.", []
