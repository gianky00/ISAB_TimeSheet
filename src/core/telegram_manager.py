...
class TelegramService(QObject):
    ...
    def _run_async_loop(self, token):
        try:
            ...
            if self.loop and self.loop.is_running(): self.loop.close()

    def _get_main_keyboard(self):
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("🛡️ PDL", callback_data="menu_pdl"), InlineKeyboardButton("📥 Scarico TS", callback_data="menu_ts")],
            [InlineKeyboardButton("📋 Dettagli OdA", callback_data="menu_oda_details"), InlineKeyboardButton("📤 Carico TS", callback_data="menu_carico")],
            [InlineKeyboardButton("⏱️ Timbrature", callback_data="menu_timbrature"), InlineKeyboardButton("📊 Stato", callback_data="status")],
            [InlineKeyboardButton("📸 Screenshot", callback_data="screenshot"), InlineKeyboardButton("⚙️ Impostazioni", callback_data="menu_settings")],
            [InlineKeyboardButton("⚡ Manutenzione", callback_data="menu_power"), InlineKeyboardButton("🛑 Stop Globale", callback_data="stop_all")]
        ])

    async def _check_auth(self, update: Update) -> bool:
        user_id = str(update.effective_user.id)
        if self.connected_chat_id and user_id != self.connected_chat_id:
            try: await update.message.reply_text("⛔ Accesso Negato")
            except: pass
            return False
        return True
    ...
    async def _cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("🚀 *SyncroJob Command Center*", reply_markup=self._get_main_keyboard(), parse_mode=constants.ParseMode.MARKDOWN)
    ...
    async def _handle_text_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if state == "WAITING_PDL": self.data_received.emit("pdl", items)
        elif state == "WAITING_ODA": self.data_received.emit("oda", items)
    ...
    async def _process_with_ai(self, chat_id, data, is_audio=False):
        ...
        def run():
            try:
                ...
                prompt = ("Sei l'interfaccia NLU di SyncroJob. Analizza il messaggio e restituisci SOLO JSON.\n" 
                ...
                res = client.analyze_media(data, prompt, "audio/ogg") if is_audio else client.ask(f"{prompt}\nMessaggio: {data}")
                try:
                    clean = res.replace("```json", "").replace("```", "").strip()
                    intent = json.loads(clean)
                    self.intent_received.emit(str(chat_id), intent)
                except: self.send_message_sync(f"Comando non riconosciuto: {res}")
            except Exception as e: self.send_message_sync(f"❌ Errore AI: {e}")
    ...
    async def _handle_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "menu_main":
            ...
            keyboard = [[InlineKeyboardButton("➕ OdA", callback_data="input_oda")], [InlineKeyboardButton("📋 Lista", callback_data="list_ts"), InlineKeyboardButton("🗑️ Svuota", callback_data="clear_ts")], [InlineKeyboardButton("▶ Avvia", callback_data="run_ts")], [InlineKeyboardButton("🔙 Indietro", callback_data="menu_main")]]
            ...
            keyboard = [[InlineKeyboardButton("➕ OdA", callback_data="input_oda")], [InlineKeyboardButton("📋 Lista", callback_data="list_ts"), InlineKeyboardButton("🗑️ Svuota", callback_data="clear_ts")], [InlineKeyboardButton("▶ Avvia Dettagli", callback_data="run_oda_details")], [InlineKeyboardButton("🔙 Indietro", callback_data="menu_main")]]
            ...
            keyboard = [[InlineKeyboardButton("▶ Avvia Carico", callback_data="run_carico")], [InlineKeyboardButton("🔙 Indietro", callback_data="menu_main")]]
            await query.edit_message_text("📤 *Carico TS*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=constants.ParseMode.MARKDOWN)
        elif data == "menu_timbrature":
            keyboard = [[InlineKeyboardButton("🕒 Ieri", callback_data="run_timbrature_yesterday")], [InlineKeyboardButton("📅 Oggi", callback_data="run_timbrature_today")], [InlineKeyboardButton("🔙 Indietro", callback_data="menu_main")]]
            await query.edit_message_text("⏱️ *Timbrature*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=constants.ParseMode.MARKDOWN)
        elif data == "input_pdl":
        ...
        elif data == "run_pdl_on": self.command_received.emit("run_pdl", {"print": True})
        elif data == "run_pdl_off": self.command_received.emit("run_pdl", {"print": False})
            ...
            keyboard = [[InlineKeyboardButton("🖼️ App", callback_data="snap_app"), InlineKeyboardButton("🖥️ PC", callback_data="snap_pc")], [InlineKeyboardButton("🔙 Indietro", callback_data="menu_main")]]
            ...
            keyboard = [[InlineKeyboardButton("🔄 Riavvia App", callback_data="app_restart")], [InlineKeyboardButton("🔌 Test Net", callback_data="app_conn_test")], [InlineKeyboardButton("🔙 Indietro", callback_data="menu_main")]]
            ...
            keyboard = [[InlineKeyboardButton("🔄 Toggle", callback_data="toggle_autopilot")], [InlineKeyboardButton("🕒 Orario", callback_data="input_autopilot_time")], [InlineKeyboardButton("🔙 Indietro", callback_data="menu_settings")]]
            ...
            await query.edit_message_text("Seleziona stampante per avviare:", reply_markup=InlineKeyboardMarkup(keyboard))
            ...
            for p in get_installed_printers():
                if p.startswith(short_p): full_p = p; break
    ...
    def send_message_sync(self, message: str):
        if self.loop and self.loop.is_running() and self.connected_chat_id:
            asyncio.run_coroutine_threadsafe(self._send_message_async(self.connected_chat_id, message), self.loop)

    def send_photo_sync(self, photo_bytes: bytes, caption: str = ""):
        if self.loop and self.loop.is_running() and self.connected_chat_id:
            asyncio.run_coroutine_threadsafe(self._send_photo_async(self.connected_chat_id, photo_bytes, caption), self.loop)

    def send_document_sync(self, file_path: str, caption: str = ""):
        if self.loop and self.loop.is_running() and self.connected_chat_id:
            asyncio.run_coroutine_threadsafe(self._send_document_async(self.connected_chat_id, file_path, caption), self.loop)

    async def _send_message_async(self, chat_id, text):
        if self.app:
            try:
                if not self.app.bot: await self.app.initialize()
                await self.app.bot.send_message(chat_id=chat_id, text=text, parse_mode=constants.ParseMode.MARKDOWN if "*" in text else None)
            except: pass
    ...
    async def _send_photo_async(self, chat_id, photo_bytes, caption):
        if self.app:
            try:
                if not self.app.bot: await self.app.initialize()
                await self.app.bot.send_photo(chat_id=chat_id, photo=photo_bytes, caption=caption, parse_mode=constants.ParseMode.MARKDOWN if caption else None)
            except: pass
        ...
        if self.app:
            try:
                ...
                with open(file_path, 'rb') as f:
                    await self.app.bot.send_document(chat_id=chat_id, document=f, caption=caption, parse_mode=constants.ParseMode.MARKDOWN if caption else None)
            except: pass
