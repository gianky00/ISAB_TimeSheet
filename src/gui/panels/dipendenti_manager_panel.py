...
from PyQt6.QtWidgets import (
    ...
    QHeaderView, QLabel, QLineEdit, QMessageBox, QDialog, QFormLayout,
    QDialogButtonBox, QFrame, QAbstractItemView, QFileDialog
)
...
class DipendentiManagerPanel(QWidget):
    ...
    def _setup_table(self):
        ...
        self.table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents) # ID stretto
        self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents) # Badge stretto
    ...
    def _sync_from_csv(self):
        ...
        try:
            ...
            AuditManager.instance().log_action("Sync CSV", "dipendenti", "Manuale", {"file": file_path, "count": count})
        except Exception as e:
    ...
    def _add_employee(self):
        ...
        if dlg.exec():
            ...
            if employee_manager.add_employee(data):
                self.refresh_data()
                self.data_changed.emit()
                AuditManager.instance().log_action("Aggiunta Dipendente", "dipendenti", f"{data['cognome']} {data['nome']}")
            else:
                QMessageBox.warning(self, "Errore", "Errore durante l'inserimento nel DB (forse ID duplicato?)")
