...
class HealthScoreBadge(QWidget):
    ...
    def _get_color(self) -> QColor:
        ...
        if self._score >= 80: return QColor("#28a745")
        if self._score >= 60: return QColor("#ffc107")
        if self._score >= 40: return QColor("#fd7e14")
        return QColor("#dc3545")

    def _get_status_text(self) -> str:
        """Restituisce la label testuale associata al punteggio."""
        if self._score >= 80: return "OTTIMO"
        if self._score >= 60: return "DISCRETO"
        if self._score >= 40: return "ATTENZIONE"
        return "CRITICO"
    ...
    def paintEvent(self, event) -> None:
        ...
        painter.setPen(QColor("#343a40")); painter.setFont(QFont("Segoe UI", 36, QFont.Weight.Bold))
...
class StatCard(QFrame):
    ...
    def _setup_ui(self, title: str, value: str, icon: str, color: str) -> None:
        ...
        lbl_title = QLabel(title); lbl_title.setStyleSheet("color: #6c757d; font-size: 12px; font-weight: 500; border: none;"); header.addWidget(lbl_title)
    ...
    def set_value(self, value: str) -> None:
        ...
        if self._value_label: self._value_label.setText(value)
...
class AnomalyCard(QFrame):
    ...
    def _setup_ui(self, anomaly) -> None:
        ...
        header.addWidget(lbl_sev); layout.addLayout(header)

        if anomaly.suggestion:
            lbl_sug = QLabel(f"💡 {anomaly.suggestion}"); lbl_sug.setStyleSheet("color: #495057; font-size: 12px; padding-left: 24px;"); lbl_sug.setWordWrap(True)
...
class HealthPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        super().__init__(parent)
        self._setup_ui()
        self._refresh_timer = QTimer(self); self._refresh_timer.timeout.connect(self.refresh); self._refresh_timer.start(120000)
        self._alert_timer = QTimer(self); self._alert_timer.timeout.connect(self._auto_check_alerts); self._alert_timer.start(1800000)
    ...
    def _setup_ui(self) -> None:
        ...
        header = QHBoxLayout()
        title = QLabel("📊 System Health"); title.setStyleSheet("font-size: 24px; font-weight: bold; color: #343a40; border: none;")
        header.addWidget(title); header.addStretch()

        self._alert_btn = ModernButton("📢 Invia Alert", variant=ModernButton.Variant.SECONDARY); self._alert_btn.clicked.connect(self._send_telegram_alert)
        header.addWidget(self._alert_btn)
        self._refresh_btn = ModernButton("🔄 Aggiorna"); self._refresh_btn.clicked.connect(self.refresh); header.addWidget(self._refresh_btn)
        layout.addLayout(header)

        content = QHBoxLayout(); content.setSpacing(24)
        left_panel = QVBoxLayout(); left_panel.setSpacing(20)
        score_card = QFrame(); score_card.setStyleSheet("QFrame { background-color: #ffffff; border-radius: 16px; border: 1px solid #dee2e6; }")
        score_layout = QVBoxLayout(score_card)
        self._score_badge = HealthScoreBadge(size=160); score_layout.addWidget(self._score_badge, alignment=Qt.AlignmentFlag.AlignCenter)
        self._status_label = QLabel("OTTIMO"); self._status_label.setAlignment(Qt.AlignmentFlag.AlignCenter); score_layout.addWidget(self._status_label)
        self._last_update = QLabel("Ultimo aggiornamento: --"); self._last_update.setAlignment(Qt.AlignmentFlag.AlignCenter); score_layout.addWidget(self._last_update)
        ...
        stats_grid.addWidget(self._stat_runs_ok, 0, 0); stats_grid.addWidget(self._stat_runs_fail, 0, 1)
        stats_grid.addWidget(self._stat_error_rate, 1, 0); stats_grid.addWidget(self._stat_anomalies, 1, 1)
        left_panel.addLayout(stats_grid); left_panel.addStretch(); content.addLayout(left_panel, stretch=1)

        right_panel = QVBoxLayout(); right_panel.setSpacing(16)
        anom_header = QHBoxLayout(); anom_header.addWidget(QLabel("⚠️ Anomalie Rilevate"))
        self._anomaly_count_label = QLabel("0 problemi"); anom_header.addStretch(); anom_header.addWidget(self._anomaly_count_label)
        ...
        self._anomalies_container = QWidget(); self._anomalies_layout = QVBoxLayout(self._anomalies_container)
        self._anomalies_layout.addStretch(); scroll.setWidget(self._anomalies_container); right_panel.addWidget(scroll)
        content.addLayout(right_panel, stretch=2); layout.addLayout(content)
    ...
    def refresh(self) -> None:
        ...
        except Exception as e: self._last_update.setText(f"Errore: {str(e)[:30]}")

    def _update_anomalies(self, anomalies: list) -> None:
        """Rigenera dinamicamente la lista delle card anomalia nella colonna di destra."""
        while self._anomalies_layout.count() > 1:
            if (item := self._anomalies_layout.takeAt(0)) and (w := item.widget()): w.deleteLater()
        self._anomaly_count_label.setText(f"{len(anomalies)} problema{'i' if len(anomalies) != 1 else ''}")
        if not anomalies:
            empty = QFrame(); empty.setStyleSheet("QFrame { background: #e8f5e9; border-radius: 12px; border: 1px dashed #81c784; }")
            el = QVBoxLayout(empty); el.addWidget(QLabel("✨"), alignment=Qt.AlignmentFlag.AlignCenter)
            el.addWidget(QLabel("Nessuna anomalia"), alignment=Qt.AlignmentFlag.AlignCenter)
            self._anomalies_layout.insertWidget(0, empty)
        else:
            for a in anomalies: self._anomalies_layout.insertWidget(self._anomalies_layout.count() - 1, AnomalyCard(a))
    ...
    def _send_telegram_alert(self) -> None:
        ...
        except Exception as e: self._show_toast(f"❌ Errore invio: {str(e)[:50]}", "error")
