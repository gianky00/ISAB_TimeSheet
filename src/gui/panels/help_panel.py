...
class HelpPanel(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        title.setStyleSheet(f"color: {COLORS['bg_white']}; font-size: 26px; font-weight: 900; letter-spacing: 0.5px;")

        subtitle = QLabel(f"SyncroJob Enterprise v{VERSION} • Hub di Supporto Tecnico")
        subtitle.setStyleSheet(f"color: {COLORS['teal_accent']}; font-size: 14px; font-weight: 600; text-transform: uppercase;")
        ...
        sidebar.setStyleSheet(f"background-color: {COLORS['bg_light']}; border-right: 1px solid {COLORS['border_light']};")
        ...
        search_container.setStyleSheet(f"background: {COLORS['bg_white']}; border: 1px solid {COLORS['border_medium']}; border-radius: 10px;")
        search_h_layout = QHBoxLayout(search_container)
        search_h_layout.setContentsMargins(12, 0, 12, 0)

        search_icon = QLabel()
        search_icon.setPixmap(get_colored_icon(get_asset_path(Icons.SEARCH), COLORS["text_muted"]).pixmap(16, 16))
        ...
        sb_footer.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 11px; font-weight: 700; text-transform: uppercase;")
