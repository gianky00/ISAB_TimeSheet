...
class MainWindow(QMainWindow):
        ...
        try:
            ...
            if mode == "app":
                ...
                for screen in screens:
                    ...
                    painter.drawPixmap(screen.geometry().topLeft() - total_rect.topLeft(), screen_pixmap)
                painter.end()
            ...
            self.telegram.send_photo_sync(
                photo_bytes, caption=f"📸 **Screenshot: {caption_text}**\nAcquisito alle {timestamp}"
            )
        except Exception as e:
    ...
    def _handle_telegram_ai_query(self, chat_id, query):
        ...
        if not api_key:
            self.telegram.send_message_sync(
                "⚠️ AI Coach non configurato. Inserisci la Gemini API Key nelle impostazioni del PC."
            )
            return
    ...
    def _perform_global_search(self):
        ...
        try:
            ...
            if ext_matches.get("GIORNALIERE"):
                ...
                for g in ext_matches["GIORNALIERE"][:20]:
                    ...
                    action.triggered.connect(
                        lambda _, q=query: self._navigate_to_extended(1, q)
                    )  # Tab 1 = Giornaliere
                    found_count += 1
                results_menu.addSeparator()
            ...
            if ext_matches.get("CERTIFICATI"):
                ...
                for c in ext_matches["CERTIFICATI"][:20]:
                    ...
                    action = results_menu.addAction(text)
                    action.triggered.connect(
                        lambda _, q=query: self._navigate_to_extended(3, q)
                    )  # Tab 3 = Certificati
                    found_count += 1
                results_menu.addSeparator()
        ...
        try:
            ...
            matches = [
                l
                for l in audit_logs
                if query.lower() in str(l["action"]).lower() or query.lower() in str(l["entity"]).lower()
            ]
            if matches:
