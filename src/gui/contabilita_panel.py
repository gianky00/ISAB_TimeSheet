...
class ContabilitaPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        selection_layout.setContentsMargins(0, 0, 10, 0); selection_layout.setSpacing(15)
        ...
        selection_layout.addWidget(self.selection_count_label); selection_layout.addWidget(self.selection_sum_label)
        self.main_tabs.setCornerWidget(self.selection_container, Qt.Corner.TopRightCorner)

        self.year_tabs_widget = QTabWidget()
        self.year_tabs_widget.setTabPosition(QTabWidget.TabPosition.South); self.year_tabs_widget.setStyleSheet(self._get_subtab_style())
        ...
        self.giornaliere_tabs_widget.setTabPosition(QTabWidget.TabPosition.South); self.giornaliere_tabs_widget.setStyleSheet(self._get_subtab_style())
    ...
    def _create_tab_wrapper(self, content_widget, placeholder_text):
        wrapper = QWidget(); layout = QVBoxLayout(wrapper); layout.setContentsMargins(10, 10, 10, 10)
        toolbar = QHBoxLayout()
        search_input = QLineEdit(); search_input.setPlaceholderText(placeholder_text); search_input.setClearButtonEnabled(True); search_input.setFixedWidth(400)
        search_input.setStyleSheet("QLineEdit { border: 1px solid #ced4da; border-radius: 4px; padding: 6px 12px; font-size: 14px; background-color: white; color: black; } QLineEdit:focus { border-color: #0d6efd; }")
        ...
        toolbar.addWidget(search_input); toolbar.addStretch()
        status_lbl = QLabel(self._last_status_html); status_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        status_lbl.setStyleSheet("QLabel { color: #495057; font-size: 14px; font-weight: 500; padding: 5px 10px; background-color: #f8f9fa; border-radius: 4px; border: 1px solid #dee2e6; }")
        self.status_labels.append(status_lbl); toolbar.addWidget(status_lbl); toolbar.addStretch()
        update_btn = QPushButton("🔄 Aggiorna Dati"); update_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        update_btn.setStyleSheet("QPushButton { background-color: #0d6efd; color: white; border: none; border-radius: 4px; padding: 6px 12px; font-weight: bold; font-size: 13px; } QPushButton:hover { background-color: #0b5ed7; }")
        update_btn.clicked.connect(self.start_import_process); self.update_buttons.append(update_btn); toolbar.addWidget(update_btn)
        layout.addLayout(toolbar); layout.addWidget(content_widget)
        return wrapper

    def _proxy_filter(self, widget, text):
        target = widget.currentWidget() if isinstance(widget, QTabWidget) else widget
        if hasattr(target, "filter_data"): target.filter_data(text)
    ...
    def _on_main_tab_changed(self, index):
        if "Analisi KPI" in self.main_tabs.tabText(index): self.selection_container.hide()
        else: self.selection_container.show(); self._connect_selection_signal()

    def refresh_tabs(self):
        curr_dati = self.year_tabs_widget.tabText(self.year_tabs_widget.currentIndex())
        curr_giorn = self.giornaliere_tabs_widget.tabText(self.giornaliere_tabs_widget.currentIndex())
        self.year_tabs_widget.clear(); self.giornaliere_tabs_widget.clear()
        years = ContabilitaManager.get_available_years()
        if not years:
            no_data = QLabel("Nessun dato disponibile."); no_data.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.year_tabs_widget.addTab(no_data, "Info"); return
        for year in years:
        ...
        for i in range(self.year_tabs_widget.count()):
            if self.year_tabs_widget.tabText(i) == curr_dati: self.year_tabs_widget.setCurrentIndex(i); break
        for i in range(self.giornaliere_tabs_widget.count()):
            if self.giornaliere_tabs_widget.tabText(i) == curr_giorn: self.giornaliere_tabs_widget.setCurrentIndex(i); break
        self._connect_selection_signal()
        if hasattr(self, "kpi_panel"): self.kpi_panel.refresh_years()
        if hasattr(self, "attivita_widget"): self.attivita_widget.refresh_data()
        if hasattr(self, "certificati_widget"): self.certificati_widget.refresh_data()

    def set_search_query(self, query):
        search_edit = self.main_tabs.currentWidget().findChild(QLineEdit)
        if search_edit: search_edit.setText(query); search_edit.setFocus(); search_edit.selectAll()

    def _on_tab_changed(self, index): self._connect_selection_signal()

    def _connect_selection_signal(self):
        curr = self.main_tabs.currentWidget()
        target = None
        if curr == self.tab_preventivi: target = self.year_tabs_widget.currentWidget()
        ...
        if target:
            if hasattr(target, "table"):
                ...
                target.table.selectionModel().selectionChanged.connect(lambda s, d: self._update_selection_total(target.table))
    ...
    def start_import_process(self):
        ...
        if not path or not os.path.exists(path):
            for lbl in self.status_labels: lbl.setText("⚠️ File non trovato.")
            return
        for btn in self.update_buttons: btn.setDisabled(True)
        for lbl in self.status_labels: lbl.setText("🔄 Aggiornamento...")
        self.worker = ContabilitaWorker(path, config.get("giornaliere_path", ""), config.get("attivita_programmate_path", ""), config.get("certificati_campione_path", ""))
    ...
    def _update_all_status_labels(self, text):
        for lbl in self.status_labels: lbl.setText(text)
    ...
    def _on_import_finished(self, success, msg, added, removed, duration):
        if success:
            ...
            for lbl in self.status_labels: lbl.setText(status)
            self.refresh_tabs()
        else:
            for lbl in self.status_labels: lbl.setText(f"❌ Errore: {msg}")
            QMessageBox.warning(self, "Errore", msg)
        self.worker = None
        for btn in self.update_buttons: btn.setDisabled(False)
