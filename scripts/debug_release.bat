...
.venv\Scripts\python.exe admin/pre_flight_check.py --help 2>&1 | findstr /i "usage error" || echo     (nessun output help)
echo     Exit code: %errorlevel%
echo.

echo     --- sync_requirements.py ---
.venv\Scripts\python.exe admin/sync_requirements.py
echo     Exit code: %errorlevel%
echo.

echo [6] Test COMPLETO release.py (con --no-git per sicurezza)...
echo ============================================
.venv\Scripts\python.exe admin/release.py auto --deploy --no-git
echo ============================================
echo.
