...
Legge la versione corrente direttamente da ``src/core/version.py`` (via AST, senza
...
OUTPUT_FILE = ROOT / ".ai-context.json"
...
def _build_context(version: str) -> dict:  # type: ignore[type-arg]
    ...
    return {
        ...
        "$generated_by": "tools/generate_ai_context.py",
        ...
        "architectural_style": "Controller-Service-Repository (SRP)",
        # ── Struttura Principale ──────────────────────────────────────────────
        "source_layout": {
            "src/core/": "Business logic, Services, Repositories — NESSUNA GUI",
            "src/gui/": "Widget, Panel, Dialog PySide6 — NESSUNA logica di business",
            "src/bots/": "Automazione Selenium/Playwright — NO import da src/gui/",
            "src/models/": "Modelli Pydantic e DTO di scambio dati",
            "src/utils/": "Utility pure (stateless) senza side-effect",
            "tools/": "Script di sviluppo (generatori, linter, schemi)",
            "tests/": "Suite pytest (unit, gui, integration, slow)",
        },
        # ── Singleton e Contratti Formali ────────────────────────────────────
        "core_singletons": {
            "settings": "src/core/config/settings.py:settings  # SyncroJobSettings Pydantic — usa SEMPRE questo, mai istanziare direttamente",
        },
        "core_interfaces": {
            "BotProtocol": "src/core/interfaces.py  # run(*args,**kwargs)->bool | force_stop()->None | cleanup()->None",
            "DataImporterProtocol": "src/core/interfaces.py  # import_file(file_path:str,...)->Any",
        },
        # ── Catalogo Bot ──────────────────────────────────────────────────────
        "bots": {
            "base_classes": {
                "SeleniumBaseBot": "src/bots/base/selenium_base_bot.py",
                "PlaywrightBaseBot": "src/bots/base/playwright_base_bot.py",
            },
            "portale_fornitori": {
                "CaricoTSBot": "src/bots/portale_fornitori/carico_ts/",
                "ScaricoTSBot": "src/bots/portale_fornitori/scarico_ts/",
                "DettagliOdABot": "src/bots/portale_fornitori/dettagli_oda/",
                "PrenotaBPBot": "src/bots/portale_fornitori/prenota_bp/",
                "TimbratureBot": "src/bots/portale_fornitori/timbrature/",
            },
            "safework": {
                "SafeWorkPDLSearchBot": "src/bots/safework/pdl/",
                "SafeWorkProgrammazioneBot": "src/bots/safework/programmazione/",
                "SafeWorkProgrammazioneSyncBot": "src/bots/safework/programmazione_sync/",
            },
            "rule": "Tutti i bot DEVONO conformarsi a BotProtocol (src/core/interfaces.py).",
        },
        # ── Database SQLite ───────────────────────────────────────────────────
        "databases": {
            "contabilita.db": {
        ...
        "development_commands": {
            ...
            "cohesion_check": "poetry run python scripts/check_cohesion.py",
            "quality_check_all": "poetry run python scripts/quality_check.py",
            "pre_commit": "poetry run pre-commit run --all-files",
            "generate_schemas": "poetry run python tools/generate_schemas.py",
            "generate_ai_context": "poetry run python tools/generate_ai_context.py",
            "generate_ci_context": "poetry run python tools/generate_ci_context.py",
        ...
        "anti_breakage_rules": [
            ...
            "SETTINGS: Usa sempre il Singleton `from src.core.config.settings import settings`. MAI istanziare SyncroJobSettings direttamente.",
        ...
        "quality_gates": {
            ...
            "cohesion": "LCOM < 40% per moduli core (vedi scripts/check_cohesion.py)",
        },
        # ── Portali Esterni ───────────────────────────────────────────────────
        "external_portals": {
